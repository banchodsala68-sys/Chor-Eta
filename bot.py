...
async def get_str_callback(event):
    ...
    await bot.send_file(event.sender_id, session_file, caption=f"🔑 Session String\n\n👤 {fname}\n🆔 `{user_id}`")
...
async def message_handler(event):
    ...
    # Check if in login process
    if user_id not in login_sessions:
        # Check for broadcast reply (admin only)
        if is_admin(user_id) and event.is_reply:
            ...
            if replied_msg.text and "BROADCAST MESSAGE" in replied_msg.text:
                ...
                await event.respond(f"📢 Broadcasting to {len(users)} users...")
